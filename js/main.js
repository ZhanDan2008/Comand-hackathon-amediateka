...
async function registerUser(e) {
  e.preventDefault();
  if(username_value.value==="admin@gmail.com" && password_value.value==="chocolate" && age_value.value==="" && password_conf_value.value===""){
    let userObj = {
...
async function login_function(){
    let all_users = await get_all_users("users")
    let a = all_users.filter(item => item.username === login_username_value.value && item.password === login_password_value.value)
    if(a.length === 0){
      place_for_errors_in_login.innerText = "Нет такого пользователя.\nНеверный пароль или имя!"
      return;
    }
